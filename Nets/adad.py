...
class AdalineGD(object):
    def _init_(self,eta = 0.0001,n_iter=10000):
        self.eta = eta
        self.n_iter = n_iter
    def fit(self,X,y):
...
X = df.iloc[0:931,[0,1,2,3,4,8,9]].values
y = df.loc[0:931,7].values
y_max = y.max()
y = y/y_max
print(y)

import matplotlib.pyplot as plt
X[:,6] = np.where((X[:,6] == '国产')| (X[:,6]=='国产（上映中……）'),0,1)
#print(X[5:10])
"""
发行类别的初始化
"""
...
for i in range(931):
    ...
    if X[:,4][i].find("科幻")>=0:
        X[:, 4][i] = 1
    elif X[:,4][i].find("动作")>=0:
        X[:, 4][i] = 1
    elif X[:,4][i].find("剧情")>=0:
        X[:, 4][i] = 1
    elif X[:, 4][i].find("喜剧") >= 0:
        X[:, 4][i] = 1
    elif X[:, 4][i].find("爱情") >= 0:
        X[:, 4][i] = 1
    else:
        X[:, 4][i] = 0
